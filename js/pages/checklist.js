/* ═══════════════════════════════════════════════════
   PANORAMA TRADE — js/pages/checklist.js
   Tela Checklist Pré-trade e Painel de Risco.
...
const ChecklistPage = (() => {
  ...
  function render(state) {
    ...
    screen.innerHTML = `
      <div class="header">
        <div>
          <div class="header-logo">Checklist <span>Pré-trade</span></div>
      ...
      <div class="score-card">
        <div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.8px;margin-bottom:9px">
          Painel de Risco Manual
        </div>
        <div class="score-hdr">
          <div>
            <div style="font-size:10px;color:var(--text3);margin-bottom:3px">Classificação</div>
            <span class="risk-badge rmed" id="risk-lbl">—</span>
          </div>
          <div class="score-num" id="risk-score" style="color:var(--orange)">0</div>
        </div>
        <div class="bar-bg">
          <div class="bar-fill" id="risk-bar" style="width:0%;background:var(--orange)"></div>
        </div>
  ...
  function toggleCheck(id, el) {
    ...
    el.innerHTML = state.checkState[id]
      ? '<i class="ti ti-check" style="font-size:12px;color:white"></i>'
      : '';
  }
  ...
  function evaluate() {
    ...
    el.innerHTML = `
      <div style="background:${result.bg};border-radius:var(--radius);padding:14px;text-align:center">
